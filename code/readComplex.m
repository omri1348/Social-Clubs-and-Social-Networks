function [config, noMap] = readComplex(config, sheetNum)
    ...
    if isequaln(rawXL,{NaN})
        ...
        %%config.Environment.number_of_agents = rawXL{9,2};
        noMap = (size(rawXL,2) < 4) || isempty(isStrInCellArray('Agent',rawXL(:,4)));
